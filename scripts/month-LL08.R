...
met <- met_year(rs_q2_metadata, "LL", 2008)
# loop to create individual month data frames
for (month in unique(met$Month)) {
  df <- XY_month(rs_q2_metadata, "LL", 2008, month)
...
# save plot
pdf(file = "plots/core_LL2008_step_envM.pdf")
# make plot
...
# save plot
pdf(file = "plots/nc_LL2008_step_envM.pdf")
# make plot
lapply(step.env, plot)
dev.off()
...
# save plot
pdf(file = "plots/LL2008_step_envM.pdf")
# make plot
lapply(step.env, plot)
dev.off()
